...
public class Worker extends Thread{
    static private String ConfigurePath = "/home/czc/pagerank/worker.config";
    //Signals
    ...
    public void SendDataToWorkers(HashMap<String, Double> current) {
        ...
        for (i = WorkerId + 1; i <= WorkerNumber; i++) {
            ...
            try {
                ...
                int otherworker = Integer.parseInt(input[0]);
                int status = Integer.parseInt(input[1]);
                OtherWorkerData[otherworker] = new HashMap<String, Double>();
                while (status != -1) {
                    for (i = 2; i < input.length; i += 2) {
                        String key = input[i];
                        Double value = Double.parseDouble(input[i + 1]);
    ...
    */
    public void work(){
        ...
        while (true) {
            ...
            try{
                System.out.println("Sleep 10s, Test Crash 1");
            ...
            try{
                System.out.println("Sleep 10s, Test Crash 2");
                Thread.currentThread().sleep(10000);
            }catch(InterruptedException ie){
                ie.printStackTrace();
            }
